...
class EmbeddedChunk:
    ...
    Attributes:
        ...
        dense: Dense embedding vector. The BM25 sparse vector is computed
            server-side by Qdrant from the chunk text at upsert time.
    """
...
class IngestionReport:
    """Outcome of a batch ingestion (one manifest), file by file.

    Attributes:
        ingested: Names of files newly indexed.
        skipped: Names of files left untouched (already indexed or unsupported).
        failed: ``(file name, error message)`` pairs for files that raised.
    """
